...
export default function GuideEditor({ sections }: Props) {
  ...
  const [editEmoji, setEditEmoji] = useState<Record<string, string>>({})
      ...
      {sections.map(section => (
        <div key={section.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {editingId === section.id ? (
            ...
            <form
              action={async (fd) => {
                fd.set('emoji', editEmoji[section.id] ?? section.emoji)
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <EmojiPicker
                  value={editEmoji[section.id] ?? section.emoji}
                  onChange={e => setEditEmoji(prev => ({ ...prev, [section.id]: e }))}
                />
                <input
                  name="title"
              ...
              <textarea
                ...
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm leading-relaxed resize-y focus:outline-none focus:ring-2 focus:ring-gray-900 placeholder:text-gray-300"
              />
              ...
              <div className="flex items-center gap-2 pt-1">
                <button type="submit" className="bg-gray-900 text-white text-sm font-medium px-5 py-2 rounded-xl hover:bg-gray-700 transition">
                  Save
                </button>
                <button type="button" onClick={() => setEditingId(null)} className="text-sm text-gray-400 hover:text-gray-700 transition px-3 py-2">
                  Cancel
                </button>
              </div>
            </form>
